...
func _check_move(input_vector):
	if input_vector.length() > cnf.MOTION_DISTANCE:
		...
		for direction in cnf.DIRECTIONS:
			# if the distance is smaller than the threshold, try to make a move
			if (direction.mouse_direction.normalized() - input_vector).length() < cnf.MINIMUM_DISTANCE_TO_MOVE:
				emit_signal("user_input", direction.matrix_direction)
				break
